...
//   六、恆粗鎖定功能：可選擇關閉面板後，依然保持過濾車種的加粗顯示
//   七、多選過濾模式：支援同時選取多種車型，點擊「全部」即可一鍵重置且不加粗
...
function _updateAllPathVisuals() {
    _allPathEls.forEach((el, pathId) => {
        ...
        // 🎯 核心判斷：只要目前有多選項目，且這台車的車種包含在內，就是被 Filter 標記
        // (如果 _activeFilters 裡面是空的，代表選了「全部」，isFiltered 就會是 false，不會加粗)
...
function _init_ui_panels() {
    ...
    filterTitle.innerHTML = '🎛️ 此頁面車種過濾 (可多選)';
    ...
    function _renderFilterList() {
        ...
        _filterCategories.forEach(cat => {
            ...
            // 🎯 判斷選取狀態：如果是「全部」，必須在沒有任何車種被選取時才會亮起
