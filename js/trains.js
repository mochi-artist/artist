...
function renderFilterPanel(trains) {
    ...
    const flexRowStyle = 'display: flex !important; flex-direction: row !important; flex-wrap: nowrap !important; justify-content: space-between !important; align-items: center !important; padding: 10px 4px !important;';

    const allFrag = tpl('tpl-filter-item');
    const allEl = allFrag.firstElementChild;
    allEl.style.cssText = `clear: both; overflow: hidden; line-height: 1; ${flexRowStyle}`;
    allEl.innerHTML = `
    ...
    CATEGORY_ORDER.filter(cat => counts[cat]).forEach(cat => {
        ...
        el.style.cssText = `clear: both; overflow: hidden; line-height: 1; ${flexRowStyle}`;
        el.innerHTML = `
...
async function init() {
    ...
    masterEl.style.cssText = `clear: both; overflow: hidden; line-height: 1; padding: 12px 4px !important; ${flexRowStyle}`;
    masterEl.innerHTML = `
    ...
    Object.keys(groups).sort().forEach(monthKey => {
        ...
        // 💡 修改吸頂高度：原本 38px -> 調整為 54px，閃過手機原生下拉選單的高度！
        headerEl.style.cssText = `clear: both; overflow: hidden; padding: 10px 4px; background-color: #0f172a; color: #e2e8f0; font-size: 0.75rem; font-weight: bold; cursor: pointer; position: sticky; top: 54px; z-index: 5; border-bottom: 1px solid #1e293b; line-height: 1; ${flexRowStyle}`;
        ...
        days.forEach(dateStr => {
            ...
            el.style.cssText = `clear: both; overflow: hidden; padding: 10px 4px !important; padding-left: 10px !important; line-height: 1; ${flexRowStyle}`;
            el.innerHTML = `
